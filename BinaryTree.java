...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        replaceValueHelper(node.left, oldVal, newVal); // Traverses the left subtree so that its balanced
        replaceValueHelper(node.right, oldVal, newVal); // Traverse the right subtree
    ...
    private int findMinHelper(Node node) {
        ...
        if (node == null) {
            return Integer.MAX_VALUE; // going to move on if null
        }
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        int count = 0;// keeps track of the count of nodes
    ...
    private int[] averageHelper(Node n) {
        ...
        if (n == null) {
            return new int[]{0, 0};  // Base case for null nodes
        }


        int[] left = averageHelper(n.left);// similiar to the GT helper with balancing out the tree
        int[] right = averageHelper(n.right);

       // getting the sum to be able to calculate the average
        int sum = n.data + left[0] + right[0];
        int count = 1 + left[1] + right[1];

        return new int[]{sum, count};  // Returning sum and count
